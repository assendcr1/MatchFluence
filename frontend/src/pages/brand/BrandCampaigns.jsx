...
import { Megaphone, Heart, Send, Download, ChevronDown, ChevronUp, RefreshCw, X } from 'lucide-react'
...
export default function BrandCampaigns() {
  ...
  const [favourites, setFavourites] = useState({}) // { campaignId: Set of influencerIds }
  ...
  const handleSend = (m, campaignTitle) => {
    const brandName = session.name || 'A brand'
    const msg = `Hi @${m.influencer?.displayName || m.displayName}! 👋\n\n${brandName} would like to invite you to collaborate on our "${campaignTitle}" campaign.\n\nTo accept this invitation and view full campaign details, please sign up to MatFluenca — Africa's Influencer Intelligence Platform:\nhttps://matfluenca.vercel.app/influencer/login\n\nWe look forward to working with you! 🚀\n\nPowered by MatFluenca | A Product of The Ablant Co.`
    navigator.clipboard.writeText(msg).then(() => {
      setCopiedId(m.influencerId)
    })
  }

  const handleExportPDF = (campaign) => {
    const matches = campaign.matchedInfluencers || []
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${campaign.title} — MatFluenca Report</title>
    <style>body{font-family:Arial,sans-serif;color:#111;padding:40px;max-width:800px;margin:0 auto}h1{font-size:24px;margin-bottom:4px}.subtitle{color:#666;font-size:14px;margin-bottom:24px}.influencer{border:1px solid #e0e0e0;border-radius:8px;padding:20px;margin-bottom:16px}.inf-header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px}.inf-name{font-size:16px;font-weight:700}.inf-score{font-size:22px;font-weight:800;color:#2563eb}.stats{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin:12px 0}.stat label{font-size:11px;color:#888;text-transform:uppercase;display:block}.stat span{font-size:14px;font-weight:600}.reason{background:#f5f5f5;border-radius:6px;padding:12px;font-size:13px;color:#444;margin-top:8px}.fav-badge{background:#fef3c7;color:#92400e;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:600}</style>
    </head><body>
    <h1>${campaign.title}</h1>
    <p class="subtitle">MatFluenca Campaign Report · ${new Date().toLocaleDateString('en-ZA')}</p>
    ${matches.map((mi, i) => {
      const inf = mi.influencer || mi
      const isFav = favourites[campaign.id]?.has(mi.influencerId)
      return `<div class="influencer">
        <div class="inf-header">
          <div><div class="inf-name">#${i+1} ${inf.displayName || inf.name} ${isFav ? '<span class="fav-badge">❤ Favourite</span>' : ''}</div><div style="color:#888;font-size:12px">${inf.niche?.nicheName || ''} · ${inf.market?.marketName || ''}</div></div>
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <p className="font-semibold" style={{ color: '#ccc' }}>{c.title}</p>
                    <p className="text-sm mt-0.5" style={{ color: '#555' }}>{c.targetPlatform} · {c.description?.slice(0, 80)}...</p>
                    <div className="flex items-center gap-4 mt-2">
                      <span className="text-xs" style={{ color: '#555' }}>{new Date(c.startDate).toLocaleDateString('en-ZA')}</span>
                      {getFavCount(c.id) > 0 && (
                      ...
                      <span className="text-xs" style={{ color: '#555' }}>{c.matchedInfluencers?.length || 0} matched influencer{c.matchedInfluencers?.length !== 1 ? 's' : ''}</span>
                    </div>
                  </div>
                ...
                <div style={{ borderTop: '1px solid #1a1a1a' }}>
                  {!c.matchedInfluencers?.length ? (
                    <div className="p-6 text-center">
                      ...
                      <button onClick={() => navigate('/brand/new-campaign')} className="btn-primary text-xs mt-3" style={{ background: '#60a5fa', color: '#0a0a0a' }}>
                        <RefreshCw size={11} /> Run New Search
                      </button>
                    </div>
                  ) : (
                    <div className="divide-y" style={{ borderColor: '#1a1a1a' }}>
                      {c.matchedInfluencers.map((mi, i) => {
                        ...
                        const isFav = favourites[c.id]?.has(mi.influencerId)
                        return (
                          <div key={mi.influencerId} className="p-5">
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold" style={{ background: 'rgba(96,165,250,0.1)', color: '#60a5fa' }}>#{i + 1}</div>
                                <div>
                                  <div className="flex items-center gap-2">
                                    <p className="font-semibold text-sm text-white">{inf.displayName || inf.name}</p>
                                    <button onClick={() => toggleFavourite(c.id, mi.influencerId)}>
                                      <Heart size={14} fill={isFav ? '#f87171' : 'none'} style={{ color: isFav ? '#f87171' : '#555' }} />
                                    </button>
                                  </div>
                            ...
                            <div className="grid grid-cols-3 gap-3 mb-3">
                              <div className="p-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #1a1a1a' }}>
                                <p className="text-xs mb-0.5" style={{ color: '#444' }}>Followers</p>
                                <p className="text-sm font-semibold text-white">{inf.followerCount?.toLocaleString()}</p>
                              </div>
                              <div className="p-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #1a1a1a' }}>
                                <p className="text-xs mb-0.5" style={{ color: '#444' }}>Engagement</p>
                                <p className="text-sm font-semibold text-white">{inf.engagementRate}%</p>
                              </div>
                              <div className="p-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #1a1a1a' }}>
                                <p className="text-xs mb-0.5" style={{ color: '#444' }}>Handle</p>
                                <p className="text-sm font-semibold text-white">{inf.instagramHandle || '—'}</p>
                              </div>
                            </div>
                            ...
                            <div className="mt-3 space-y-2">
                              <button onClick={() => handleSend(mi, c.title)} className="btn-primary text-sm" style={{ background: copiedId === mi.influencerId ? '#4ade80' : '#60a5fa', color: '#0a0a0a' }}>
